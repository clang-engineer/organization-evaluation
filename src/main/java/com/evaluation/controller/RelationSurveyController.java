...
public class RelationSurveyController {
     ...
     * 관계를 등록한다.
     * 
     * @param relationSurvey Survey 관계 정보
     * @param vo             페이지 정보
     * @param rttr           재전송 정보
     * @return 관계 목록 페이지
     */
    ...
    public void xlDownload(@PathVariable("tno") long tno, HttpServletResponse response) {
        ...
        turnService.read(tno).ifPresent(origin -> {
            ...
            header.add("계층");
            header.add("본인평가");
            origin.getSurveyAppellation().forEach(appellation -> {
            ...
            evaluatedList.forEach(evaluated -> {
                ...
                List<RelationSurvey> relationList = relationSurveyService.findAllByTno(tno).orElse(null);
                ...
                // 관계 사이즈를 기준으로 for문
                for (int i = 0; i < origin.getSurveyAppellation().size(); i++) {
