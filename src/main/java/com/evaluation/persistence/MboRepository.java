...
public interface MboRepository extends CrudRepository<Mbo, Long> {
    ...
    /**
     * recentChangeOfEvaluatedList를 위한 쿼리문
     */
    String recentChangeOfEvaluatedList = 
    "SELECT r.rno, r.name, u.index, u.type, u.write_id, u.write_date, u.update_date "+
    "FROM (SELECT r.rno, r.evaluated, s.name, r.evaluator FROM tbl_relation_mbo r "+
    "LEFT JOIN tbl_staff s ON r.evaluated=s.sno "+
    "WHERE r.turn_tno=:tno AND r.evaluator=:sno) r "+
    "LEFT JOIN "+
    ...
    "FROM evaluation.tbl_mbo m "+
    "WHERE turn_tno=:tno AND finish='Y') "+
    "UNION ALL "+
    ...
    "FROM tbl_mbo_reply r "+
    "RIGHT JOIN tbl_mbo m ON r.mbo_mno=m.mno "+
    "WHERE turn_tno=:tno AND finish='Y')) u "+
    "ON r.evaluated=u.staff_sno "+
     ...
     * 2. 회차 내에 목표와 댓글 리스트를 합친다. (최종 Y만 해당)
    ...
    Optional<List<String>> recentChangeOfEvaluatedList(@Param("tno") long tno, @Param("sno") long sno, @Param("offset") int offset);
}